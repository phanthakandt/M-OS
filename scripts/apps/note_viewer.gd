...
func _ready() -> void:
	...
	rich_text.add_theme_font_size_override("normal_font_size", 7)
	rich_text.add_theme_color_override("default_color", Palette.TEXT_DIM)


func show_warning_and_rules(warning_title: String, warning_sub: String, rules: Array) -> void:
	var bb := "[bgcolor=#2b1a1c][color=#a8484a]%s\n%s[/color][/bgcolor]\n\n" % [warning_title, warning_sub]
	for rule in rules:
